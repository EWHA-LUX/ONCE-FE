...
class _Signup2State extends State<Signup2> {
  ...
  void checkDuplicate(String loginId) async {
    final String duplicateCheckUrl = '${BASE_URL}/user/duplicate?loginId=$loginId';
    try {
      var duplicateCheckResponse = await Dio().get(duplicateCheckUrl);
      Map<String, dynamic> duplicateCheckData = duplicateCheckResponse.data;
      setState(() {
        if (duplicateCheckData['code'] == 1000) {
          idAvailable = duplicateCheckData['result'];
        }
      });
    } catch (e) {
      print(e.toString());
